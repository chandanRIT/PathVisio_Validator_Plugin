...
public class GraphAnalysisPlugin implements Plugin, ApplicationEventListener {
	...
	private void generateJGraphOutputFile(Graph<VPNode, JPEdge> jGraph) {
		...
		if (jcb1.isSelected()) {
			try {
				// a new object has to be created every time, since save method
				// below closes the streams
			...
			System.out.println("JoutputFile deleted or not:" + jGFile.delete());

	}

	private VPNode findNodebyGId(String gid) {
		if (gid == null)
			System.out.println("gid passed to findNode is null");

		for (VPNode vpn : vpnodes) {
			if (vpn.graphId == null || vpn.graphId.equals(""))
				System.out.println(vpn + " has null or empty id");
	...
	/* 
	 * Create graph from pathway
	 */
	private Graph<VPNode, JPEdge> createJGraphFromPWFile(File pwExportedFile) {
		Graph<VPNode, JPEdge> vpgraph = new SparseGraph<VPNode, JPEdge>();
		if (vpnodes == null)
			vpnodes = new ArrayList<VPNode>();
		if (sh == null)
			sh = new PathwayXMLHandler(vpnodes);
		...
		for (VPNode vpn : vpnodes) {
			...
			if (vpn instanceof VPLine) {
				...
				try {
					vpgraph.addEdge(new JPEdge(vpn.graphId,
							gId2 = ((VPLine) vpn).graphRef[0]), vpn,
							findNodebyGId(gId2));
					vpgraph.addEdge(new JPEdge(vpn.graphId,
							gId2 = ((VPLine) vpn).graphRef[1]), vpn,
							findNodebyGId(gId2));
				} catch (Exception e) {
				...
				// not needed to add the vertex since automatically added by the
				// addEdge method
				vpcn = ((VPAnchor) vpn).connectedToLine;

				vpgraph.addVertex(vpn);
				vpgraph.addEdge(new JPEdge(vpn.graphId, vpcn.graphId), vpn,
						vpcn);
	...
	private void createAndDisplayJGraphUI(Graph<VPNode, JPEdge> vpgraph) {
		...
		Layout<VPNode, JPEdge> layout = new CircleLayout<VPNode, JPEdge>(
				vpgraph);
		layout.setSize(new Dimension(950, 750)); // sets the initial size of the
													// space

		// The BasicVisualizationServer<V,E> is parameterized by the edge types
		BasicVisualizationServer<VPNode, JPEdge> vv = new BasicVisualizationServer<VPNode, JPEdge>(
				layout);
		vv.setPreferredSize(new Dimension(1000, 800)); // Sets the viewing area
														// size
		vv.getRenderContext().setVertexLabelTransformer(
				new ToStringLabeller<VPNode>());
		vv.getRenderContext().setEdgeLabelTransformer(
				new ToStringLabeller<JPEdge>());
	...
	private boolean checkForGIdValidity(VPNode vpn1, VPNode vpn2) {
		...
		if (failFlag) {
			JOptionPane.showMessageDialog(desktop.getFrame(), failMessage,
	...
	private void createGraphAndRunAlgorithm(int alBoxselection, String input1,
			String input2) {

		if (!engine.hasVPathway()) {
			JOptionPane.showMessageDialog(desktop.getFrame(),
					"Pathway not open", "JPlugin",
					JOptionPane.INFORMATION_MESSAGE);
			return;
		}
		
		// Make sure all lines have IDs 
		for(PathwayElement pw : engine.getActivePathway().getDataObjects()) {
			if(pw.getGraphId() == null) {
				pw.setGraphId(engine.getActivePathway().getUniqueGraphId());
			}
		}
		...
		if (!checkForGIdValidity(vpn1, vpn2))
			return;

		switch (alBoxselection) {
		case 0:
			NAP_Utility.alg_shortestPath_DA(createdJGraph, vpn1, vpn2);
			break;
		case 1:
			NAP_Utility
					.alg_DFS_cycleFinding(alg_findCycle, createdJGraph, vpn1);
			break;
		case 2:
			NAP_Utility.alg_minSpanForest(createdJGraph, vpn1);
			break;
		default:
			System.out.println("pressed another");
		}

		NAP_Utility.computeAndDisplayStatistics(createdJGraph, vpn1, vpn2,
				vpnodes);
	...
	private boolean exportPathwayToXML() {
		try {
			GpmlFormat.writeToXml(engine.getActivePathway(),
					currentPathwayFile, true);
		} catch (Exception e) {
			...
			JOptionPane
					.showMessageDialog(
							desktop.getFrame(),
							"GPML conversion Error (unable to covert pathway into XML formatted file )",
							"JPlugin", JOptionPane.INFORMATION_MESSAGE);
			return false;
		}
		return true;
	...
	/*
	 * This method envelopes the actual method to catch any runtime or uncaught
	 * exceptions
	 */
	private void runAction(int sel, String input1, String input2) {
		try {
			...
			JOptionPane.showMessageDialog(desktop.getFrame(),
					"Exception occured during run", "JPlugin",
					JOptionPane.INFORMATION_MESSAGE);
	...
	private void JPActionListenerMethod(ActionEvent e) {
		if (e.getSource() instanceof JTextField
				|| "run".equals(e.getActionCommand())) {
			// System.out.println("text field event");
			runAction(alBox.getSelectedIndex(), tf1.getText().trim(), tf2
					.getText().trim());
		}
		 ...
		 * runAction(alBox.getSelectedIndex(), tf1.getText().trim(),
		 * tf2.getText().trim()); }
		 */
	}
	...
	private void createJPluginUI() {
		...
		JPanel mainPanel = new JPanel(new BorderLayout()), inputPanel = new JPanel(
				new GridBagLayout());
		inputPanel.setBorder(BorderFactory.createTitledBorder(etch,
				"Input Pane"));
		...
		jcb1.setToolTipText("generated in " + System.getProperty("user.home")
				+ System.getProperty("file.separator") + "JPluginGraph.xml");
		...
		resultsPanel.setBorder(BorderFactory.createTitledBorder(etch,
				"results Pane"));
		c3.weightx = 1;
		c3.weighty = 1;
