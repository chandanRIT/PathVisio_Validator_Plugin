...
<iso:schema    
  ...
  schemaVersion="0.1">
	...
	<iso:pattern id="pd10143">
		<iso:rule context="sbgn:glyph[@class='unit of information']">
			<iso:let name="id" value="@id"/>
			<iso:let name="child-tag-name" value="../local-name()"/>
			<iso:assert 
				id="pd10143"
				name="check-glyph"
				role="error"
				test="
				$child-tag-name = 'unspecified entity' or 
				$child-tag-name = 'macromolecule' or				
				$child-tag-name = 'nucleic acid feature' or
				$child-tag-name = 'simple chemical multimer' or 
				$child-tag-name = 'macromolecule multimer' or 
				$child-tag-name = 'nucleic acid feature multimer' or 
				$child-tag-name = 'complex'  "  diagnostics="id">PARENT of state varaible  must be glyph.
			</iso:assert>
		</iso:rule> 
	</iso:pattern> 
	
	<iso:diagnostics>
	<iso:diagnostic id="child"><iso:value-of select="$child"/></iso:diagnostic>
