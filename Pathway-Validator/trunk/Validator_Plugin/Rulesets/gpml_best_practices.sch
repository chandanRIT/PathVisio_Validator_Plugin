  ...
  schemaVersion="0.1">
	...
	<iso:pattern name="check-title" id="check-title">
		<iso:rule context="gpml:Pathway" role="error">
			<iso:assert test="@Name and not(@Name='')">Diagrams should have a title.</iso:assert>
		</iso:rule> 
	</iso:pattern> 
	...
	<iso:pattern name="check-author" id="check-author">
		<iso:rule context="gpml:Pathway" role="warning">
			<iso:assert test="@Author and not(@Author='')">Diagrams should have an author.</iso:assert>
		</iso:rule> 
	</iso:pattern> 
	...
	<iso:pattern name="check-organism" id="check-organism">
		<iso:rule context="gpml:Pathway" role="warning">
			<iso:assert test="@Organism and not(@Organism='')">Diagrams should have a organism.</iso:assert>
		</iso:rule> 
	</iso:pattern> 
	...
	<iso:pattern name="check-db-xref" id="check-db-xref">
		<iso:rule context="gpml:DataNode" role="error">
			...
			<iso:assert test="gpml:Xref[not(@Database='')] and gpml:Xref[not(@ID='')]" diagnostics="graph-id">Datanodes should include database annotations.</iso:assert>
		</iso:rule> 
	</iso:pattern> 
	...
	<iso:pattern name="check-labels" id="check-labels">
		<iso:rule context="gpml:DataNode" role="error">
			...
			<iso:assert test="@TextLabel and not(@TextLabel='')" diagnostics="graph-id">DataNodes should have a text label.</iso:assert>
		</iso:rule> 
	</iso:pattern> 
